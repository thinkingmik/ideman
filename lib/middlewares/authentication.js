...
passport.use('client-password', new ClientPasswordStrategy(
  function(clientId, clientSecret, callback) {
    AutoLoader.model('Client').forge().where({
      name: clientId,
      enabled: true
    })
    .fetch({
  ...
  function(req, accessToken, callback) {
    ...
    .then(function(ret) {
      if (this.token.get('userId') != null) {
        return AutoLoader.model('User').forge()
          .where({
        ...
        return AutoLoader.model('Client').forge()
          .where({
