  ...
  return AutoLoader.getBookshelf().transaction(function (tnx) {
    ...
    .then(function(guid) {
      ...
      if (!_.isNull(userId) && !_.isUndefined(userId)) {
        ...
        .save({ lastLogin: currentDatetime }, { patch: true, transacting: tnx, require: false });
      }
      return null;
    })
    .then(function(user) {
  ...
  .then(function(jtiVal) {
    return new Promise(function(resolve, reject) {
      try {
        ...
        var claim = {
          'iss': client.get('name'),
          'sub': (!_.isNull(userId) && !_.isUndefined(userId)) ? userId : client.get('id'),
...
var uid = function(len) {
  ...
  return cryptoManager.randomBytes(len)
  .nodeify(function(re, bytes) {
    return bytes;
  });
}
...
var exchangePassword = function (client, username, password, ip, userAgent) {
  ...
  if (config.ldap.enabled === true) {
    var ldapManager = new Ldapper(config.ldap.ldapper);
    ldapPromise = ldapManager.authenticate(username, password, config.ldap.authAttributes, config.ldap.returnAttribute)
    .then(function(res) {
      if (_.isNull(res)) {
        return null;
      }
      return res[config.ldap.returnAttribute];
    });
  }
  ...
  .then(function(tokens) {
    return createTokens(client, this.user.get('id'), req, tokens);
  })
  .catch(function(err) {
    throw new AuthGrantError(err.message);
  });
}
...
var exchangeRefreshToken = function (client, refreshToken) {
  ...
  .then(function(token) {
    return createTokens(client, this.user.get('id'), this.req, token);
  })
  .catch(function(err) {
    throw err;
  });
...
var exchangeCode = function(client, code, redirectUri, ip, userAgent) {
  ...
  .then(function(code) {
    return createTokens(client, this.authCode.get('userId'), req, code);
  })
  .catch(function(err) {
    throw err;
  });
